...
class ControllerDesignBanner extends Controller {
    ...
    foreach($banner_image['banner_image_description'] as $i =>$d)
    {
    
      if ($d['image'] && file_exists(DIR_IMAGE . $d['image'])) {
				$image = $d['image'];
			} else {
				$image = 'no_image.jpg';
			}
      $thumb =$this->model_tool_image->resize($image, 100, 100);
      $arr[$i]=Array('title'=>$d['title'],'thumb'=>$thumb,'image'=>$image);
      
    }
    
    
    $banner_image['banner_image_description']=$arr;
