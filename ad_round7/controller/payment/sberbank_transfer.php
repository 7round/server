...
class ControllerPaymentSberBankTransfer extends Controller {
		...
		foreach ($languages as $language) {
			...
			if (isset($this->request->post['sberbank_transfer_inn_' . $language['language_id']])) {
				...
				$this->data['sberbank_transfer_inn_' . $language['language_id']] = $this->config->get('sberbank_transfer_inn_' . $language['language_id']);
                        ...
                        //kpp
                        if (isset($this->request->post['sberbank_transfer_kpp_' . $language['language_id']])) {
				$this->data['sberbank_transfer_kpp_' . $language['language_id']] = $this->request->post['sberbank_transfer_kpp_' . $language['language_id']];
			} else {
				$this->data['sberbank_transfer_kpp_' . $language['language_id']] = $this->config->get('sberbank_transfer_kpp_' . $language['language_id']);
			}
	...
	private function validate() {
		...
		foreach ($languages as $language) {
			...
			if (!$this->request->post['sberbank_transfer_kpp_' . $language['language_id']]) {
				$this->error['kpp_' .  $language['language_id']] = $this->language->get('error_bank');
			}
