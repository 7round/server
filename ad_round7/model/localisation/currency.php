...
class ModelLocalisationCurrency extends Model {
	...
	public function updateCurrencies($force = false) {
		if (extension_loaded('curl')) {
          ...
          if ($Valute['CharCode']=='EUR')$Valute['CharCode']='RUB';
						if ( isset($currencies[$Valute['CharCode']]) && isset($Valute['Value'])) {
							...
							if ($value) {
								$this->db->query("UPDATE " . DB_PREFIX . "currency SET value = '" . $value . "', date_modified = '" .  $this->db->escape($date_modified) . "' WHERE code = '" . $this->db->escape($Valute['CharCode']) . "'");
    					}
