<?php
/**
* Yandex.YML data feed for OpenCart (ocStore) (1.5.5.x - 2.x)
*
 ...
 */
//Версия модуля
define ('YANDEX_YML_VERSION', '1.7.1');
...
//---- Config section ----
	...
	*/
	...
	protected function getYmlChunk($page = 0) {
		...
		foreach ($this->model_export_yandex_yml->getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_id, $allowed_manufacturers, $product_rel || $product_accessory, $skip, $limit) as $product) {
			$is_out = false;
			$data = array();
...
//				$data['manufacturer_warranty'] = 'true';
			...
			*/
			...
			if ($this->config->get($this->CONFIG_PREFIX.'attr_vs_description')) {
				$data['description'] = implode($attr_text,"<br/>\n");
			}
			else {
			...
			if ($this->config->get($this->CONFIG_PREFIX.'export_tags')) {
				$data['description'] = '<![CDATA['.$data['description'].']]>';
			}
			//---- Описание товара ----
